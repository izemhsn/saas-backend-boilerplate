...
export const createFlagSchema = z.object({
  body: z.object({
    key: z
      ...
      .max(100)
      .regex(/^[a-z0-9](?:[a-z0-9_:.-]*[a-z0-9])?$/, 'Key must be lowercase alphanumeric with hyphens, dots, colons, or underscores'),
    ...
    if (data.type === 'BOOLEAN' && typeof data.value?.enabled !== 'boolean' && Object.keys(data.value).length > 0) {
      ctx.addIssue({ path: ['body', 'value'], message: 'BOOLEAN flags use { enabled: true/false }' })
    }
    if (data.type === 'PERCENTAGE') {
      const pct = data.value?.percentage
      if (pct !== undefined && (typeof pct !== 'number' || pct < 0 || pct > 100)) {
        ctx.addIssue({ path: ['body', 'value'], message: 'PERCENTAGE flags use { percentage: 0-100 }' })
      }
    }
    if (data.type === 'PLAN' && data.value?.plans !== undefined) {
      if (!Array.isArray(data.value.plans) || !data.value.plans.every((p) => typeof p === 'string')) {
        ctx.addIssue({ path: ['body', 'value'], message: 'PLAN flags use { plans: ["Free", "Pro"] }' })
      }
    }
  }),
})
