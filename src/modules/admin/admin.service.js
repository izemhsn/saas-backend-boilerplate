...
export const updateUser = async (userId, data, actingAdminId) => {
  if (userId === actingAdminId) {
    if (data.banned === true) throw httpError('errors.cannotBanSelf', 400)
    if (data.role !== undefined && data.role !== 'ADMIN')
      throw httpError('errors.cannotDemoteSelf', 400)
  }
  ...
  if (data.role !== undefined && data.role !== 'ADMIN' && user.role === 'ADMIN') {
    ...
    if (adminCount <= 1) throw httpError('errors.cannotDemoteLastAdmin', 400)
  }
...
export const deleteUser = async (userId, actingAdminId) => {
  ...
  if (user.role === 'ADMIN') {
    const adminCount = await prisma.user.count({ where: { role: 'ADMIN', deletedAt: null } })
    if (adminCount <= 1) throw httpError('errors.cannotDeleteLastAdmin', 400)
  }

  if (userId === actingAdminId) throw httpError('errors.cannotDeleteSelf', 400)

  await prisma.user.update({ where: { id: userId }, data: { deletedAt: new Date() } })
  return { messageKey: 'messages.userDeletedSuccessfully' }
}
